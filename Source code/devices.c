...
void init_dac_xmega( void)							// Initialize Xmega internal DAC
{
	...
	DACB_CTRLB = DAC_CHSEL_DUAL_gc;				// No Auto Trigger
	DACB_CTRLC = DAC_REFSEL1_bm;					// External reference voltage 2.5V
	DACB_TIMCTRL = 0x65;							// DAC S/H period
	DACB_CTRLA = DAC_CH0EN_bm | DAC_CH1EN_bm | DAC_ENABLE_bm;		// Set DACB channel 0 and enable
...
{
	DACB_CH1DATA = data;	// CH0 je odprasknuty !!!
